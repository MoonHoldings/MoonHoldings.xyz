...
export const useNftStore = defineStore('nft', {
  ...
  actions: {
    ...
    mutate_removePortfolio(portfolio) {
      const searchPortfolio = this.portfolios.findIndex(
        (item) => item.walletAddress === portfolio.walletAddress
      )
    ...
    async connectWalletWithAddress(walletAddress) {
      try {
        ...
        if (res.success && nfts.length > 0) {
          ...
          nfts.forEach((nft) => {
            updateAuthority =
              nft?.updateAuthorityAddress ?? 'missingUpdateAuthority'
        ...
        mixpanel.track('Error: nft.js > connectWalletWithAddress', {
          error: error,
    ...
    async connectWalletWithAddress2(walletAddress) {
      try {
        const response = await axios.get(
          `${this.shyft_url}/wallet/get_portfolio?network=mainnet-beta&wallet=${walletAddress}`,
          {
        ...
        if (res.success && nfts.length > 0) {
          ...
          if (
            raw_collections['unknown'] &&
            raw_collections['unknown'].length > 0
          ) {
          ...
          const getCollectionNameImage = (collection) => {
            ...
            if (
              collection[0].collection?.collection_name &&
              collection[0].collection?.collection_image
            ) {
              return
            }
          ...
          console.log(
            'Unknown collections (this.filtered_collections)',
            this.filtered_collections
          )
    ...
    async fetchNFT(uriAddress) {
      try {
        ...
        console.log(error)
        mixpanel.track('Error: nft.js > fetchNFT', {
          error: error,
          message: error.message,
        })
      }
    ...
    async fetchURI(uriAddress, firstNFT) {
      try {
        const response = await axios.get(`${uriAddress}`, {
        ...
        if (res) {
          for (const [key, value] of Object.entries(
            ...
            if (key === firstNFT.updateAuthorityAddress) {
              value.forEach((nft) => {
                if (
                  nft.updateAuthorityAddress === firstNFT.updateAuthorityAddress
                ) {
                  const name =
                    res.collection && res.collection.name
                      ? res.collection.name
                      : res.name
